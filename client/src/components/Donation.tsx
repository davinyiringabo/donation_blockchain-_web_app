...
const Donation: React.FC<{ darkMode: boolean }> = ({ darkMode }) => {
  ...
  const { donations, loading, success } = useAppSelector(
    ...
    <div className="mt-[70px] flex justify-center items-center">
      <div className="p-[5px] md:w-[80%] w-[100%] mb-[20px]">
        {donation ? (
          <>
            <div
              className={`${darkMode ? "text-white" : "text-[#121212]"} mb-6`}
            >
              Donation ({id})
            </div>

            <div
              className={`grid gap-4 md:grid-cols-2 sm:grid-cols-1 pb-4 mb-4 border-b border-[${darkMode ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.3)"}`}
            >
              <div className="">
                <img
                  src={donation.image}
              ...
              <div className={darkMode ? "text-white" : "text-[#121212]"}>
                <div className="font-bold mb-3">{donation.title}</div>
                ...
                <div>
                  <div className="font-bold">Days left</div>
                  <div className="text-sm p-2 text-justify">
                    {Math.ceil(
                      (new Date(
                        parseInt(donation.deadline.toString(), 10),
                      ).getTime() -
                        new Date().getTime()) /
                        (1000 * 3600 * 24),
                    ) >= 1 ? (
                ...
                <div>
                  <div className="font-bold">Collected amount</div>
            ...
            <div className="grid gap-4 md:grid-cols-2 sm:grid-cols-1 md:mb-0 sm:mb-0 mb-20 ">
              <div className={`${darkMode ? "text-white" : "text-[#121212]"}`}>
                    ...
                    <div>
                      ...
                      <div className="text-[10px]">
                        <span className="font-bold">
                          {
                            donations.filter((d) => d.owner === donation.owner)
                              .length
                          }
                        </span>{" "}
                        Donation(s)
                ...
                <div>
                  <div className="font-bold">Donators</div>
                  <div className="p-2">
                    {donation.donators.length === 0 ? (
                      ...
                      <>
                        {donation.donators.map((address, index) => (
                          <div
                            key={index}
                            className="border-b border-b-1 mb-2 flex flex-wrap justify-between items-center"
                          >
              ...
              <div className="flex justify-center">
                {Math.ceil(
                  ...
                  <div
                    className={`md:w-96 w-full ${darkMode ? "text-white" : "text-[#121212]"}`}
                  >
                    <div className="font-bold mb-3">Fund</div>

                    <div
                      className={`p-2 shadow-xl rounded-lg ${darkMode ? "bg-[#1c1c24]" : "bg-white"}`}
                    >
                      <div className="text-center mb-3">Fund now !</div>
                  ...
                  <div
                    className={`font-bold w-full text-center ${darkMode ? "text-white" : "text-[#121212]"}`}
                  >
