...
const Profile: React.FC<{ darkMode: boolean }> = ({ darkMode }) => {
  ...
  const { connected, address, balance } = useAppSelector(
    (state) => state.wallet,
  );
    ...
    <div className="mt-[70px] flex justify-center items-center">
      <div className="p-[5px] md:w-[80%] w-[100%]">
        ...
        <div className="text-sm mb-6">
          <div
            className={`flex flex-wrap items-center px-2 py-1 ${darkMode ? "text-white" : "text-[#121212]"}`}
          >
        ...
        <div
          className={`mb-6 px-3 text-sm py-1 ${darkMode ? "text-white" : "text-[#121212]"}`}
        >
          Your donations (
          {
            donations.filter(
              (e) => e.owner.toUpperCase() === address.toUpperCase(),
            ).length
          }
          )
        </div>
        ...
        <div className="grid gap-x-10 gap-y-4 md:grid-cols-3 sm:grid-cols-2 mb-16">
          {donations
            .filter((e) => e.owner.toUpperCase() === address.toUpperCase())
            .map((e: Donation, index: number) => (
              <Card darkMode={darkMode} data={e} id={index + 1} key={index} />
            ))}
        </div>
      </div>
    </div>
