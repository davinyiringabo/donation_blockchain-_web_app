...
export const getReceiverNotifications = createAsyncThunk(
  "notifications/get",
  async (receiver: string) => {
    return await axios
      .get(`http://localhost:3700/api/notification/get/${receiver}`)
...
export const saveNotification = createAsyncThunk(
  ...
  async (data: any) => {
    return await axios
      .post("http://localhost:3700/api/notification/save", data)
      .then((response) => data)
      .catch((err) => err.response.message);
  },
);
...
export const seenNotification = createAsyncThunk(
  ...
  async (id: string) => {
    return await axios
      .put(`http://localhost:3700/api/notification/seen/${id}`)
